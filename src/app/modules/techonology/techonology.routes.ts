...
import { TechonologyController } from './techonology.controller';
import { TechnologyValidation } from './techonology.validation';
 ...
 *     description: Create a new blog with thumbnail image upload (requires authentication and otherPageSetup permission)
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 *       required: true
...
router.post(
  ...
  uploadImages,
  validation(TechnologyValidation.create),
  TechonologyController.createTechonology,
);
 ...
 *     tags:
 *       - otherPageSetup
 *     summary: Get all otherPageSetup
 *     description: Retrieve a list of all otherPageSetup (public endpoint - no authentication required)
 *     parameters:
 *       - in: query
 *         name: page
 *         schema:
 ...
 *           minimum: 1
 *           maximum: 100
 *         description: Number of otherPageSetup per page
 *         example: 10
 *       - in: query
 *         name: categoryId
 *         schema:
 *           type: string
 *           format: uuid
 *         description: Filter otherPageSetup by category ID
 *         example: "123e4567-e89b-12d3-a456-426614174000"
 *       - in: query
 *         name: searchTerm
 *         schema:
 *           type: string
 *         description: Search otherPageSetup by title or description
 *         example: "web development"
 *     responses:
 *       200:
 *         description: otherPageSetup retrieved successfully
 *         content:
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/ApiResponse'
 ...
 *               success: true
 *               statusCode: 200
 *               message: "otherPageSetup retrieved successfully"
 *               data:
 *                 otherPageSetup:
 *                   - id: "123e4567-e89b-12d3-a456-426614174000"
 *                     title: "Understanding Modern Web Development"
 ...
 *               $ref: '#/components/schemas/ErrorResponse'
 */
router.get('/', TechonologyController.getAllTechonology);
 ...
 *               $ref: '#/components/schemas/ErrorResponse'
 */
router.get('/:id', TechonologyController.getSingleTechonology);
 ...
 *     description: Update an existing blog by its ID with optional thumbnail image upload (requires authentication and otherPageSetup permission)
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - in: path
...
router.put(
  ...
  imageUpload.single('icon'),
  uploadImages,
  validation(TechnologyValidation.update),
  TechonologyController.updateTechonology,
);
 ...
 *     description: Delete an existing blog by its ID (requires authentication and otherPageSetup permission)
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - in: path
 ...
 *               $ref: '#/components/schemas/ErrorResponse'
 */
router.delete('/:id', auth([featureNames.otherPageSetup]), TechonologyController.deleteTechonology);

export const TechonologyRoutes = router;
